-- Create links table for artist/label social links
CREATE TABLE social_links (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  artist_id UUID REFERENCES artists(id) ON DELETE CASCADE,
  label_id UUID REFERENCES labels(id) ON DELETE CASCADE,
  title TEXT NOT NULL,
  url TEXT NOT NULL,
  platform TEXT, -- 'spotify', 'instagram', 'tiktok', 'youtube', 'soundcloud', 'twitter', 'facebook', 'website', 'custom'
  icon_url TEXT,
  position INTEGER DEFAULT 0,
  is_visible BOOLEAN DEFAULT true,
  click_count INTEGER DEFAULT 0,
  created_at TIMESTAMPTZ DEFAULT NOW(),
  updated_at TIMESTAMPTZ DEFAULT NOW(),
  
  -- Ensure link belongs to either artist or label, not both
  CONSTRAINT link_owner CHECK (
    (artist_id IS NOT NULL AND label_id IS NULL) OR
    (artist_id IS NULL AND label_id IS NOT NULL)
  )
);

-- Indexes
CREATE INDEX idx_social_links_artist_id ON social_links(artist_id);
CREATE INDEX idx_social_links_label_id ON social_links(label_id);
CREATE INDEX idx_social_links_position ON social_links(position);
CREATE INDEX idx_social_links_visible ON social_links(is_visible);

-- Function to increment click count
CREATE OR REPLACE FUNCTION increment_link_clicks(p_link_id UUID)
RETURNS VOID AS $$
BEGIN
  UPDATE social_links
  SET click_count = click_count + 1
  WHERE id = p_link_id;
END;
$$ LANGUAGE plpgsql;

-- RLS Policies
ALTER TABLE social_links ENABLE ROW LEVEL SECURITY;

-- Public can view visible links
CREATE POLICY "Public can view visible links"
  ON social_links FOR SELECT
  USING (is_visible = true);

-- Artists can manage their own links
CREATE POLICY "Artists can manage own links"
  ON social_links FOR ALL
  USING (auth.uid() = artist_id);

-- Labels can manage their own links
CREATE POLICY "Labels can manage own links"
  ON social_links FOR ALL
  USING (auth.uid() = label_id);

-- Create table for link click analytics
CREATE TABLE link_clicks (
  id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
  link_id UUID NOT NULL REFERENCES social_links(id) ON DELETE CASCADE,
  clicked_at TIMESTAMPTZ DEFAULT NOW(),
  referrer TEXT,
  user_agent TEXT,
  ip_address INET
);

CREATE INDEX idx_link_clicks_link_id ON link_clicks(link_id);
CREATE INDEX idx_link_clicks_clicked_at ON link_clicks(clicked_at);

-- RLS for link_clicks
ALTER TABLE link_clicks ENABLE ROW LEVEL SECURITY;

-- Only service role can insert click records
CREATE POLICY "Service role can insert clicks"
  ON link_clicks FOR INSERT
  WITH CHECK (true);

-- Owners can view their link analytics
CREATE POLICY "Artists can view own link clicks"
  ON link_clicks FOR SELECT
  USING (
    EXISTS (
      SELECT 1 FROM social_links
      WHERE social_links.id = link_clicks.link_id
      AND social_links.artist_id = auth.uid()
    )
  );

CREATE POLICY "Labels can view own link clicks"
  ON link_clicks FOR SELECT
  USING (
    EXISTS (
      SELECT 1 FROM social_links
      WHERE social_links.id = link_clicks.link_id
      AND social_links.label_id = auth.uid()
    )
  );

COMMENT ON TABLE social_links IS 'Social media and external links for artist/label profiles (Link-in-Bio feature)';
COMMENT ON TABLE link_clicks IS 'Analytics tracking for social link clicks';
